...
export default function Login(){
  ...
  useEffect(() => {
    // Déclenche un timer qui exécutera `dispatch(clearMessage)` après 3 secondes
    const timer = setTimeout(() => {
      dispatch(clearMessage()); // Action qui efface le message
    }, 3000); // Délai de 3 secondes
  
    // Nettoyage du timer lorsque le composant est démonté ou si `dispatch` change
    return () => clearTimeout(timer); 
  }, [dispatch, erreur]); // La dépendance est `dispatch` (normalement stable)
